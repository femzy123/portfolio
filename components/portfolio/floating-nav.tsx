...
export function FloatingNav({
  ...
  return (
    <nav
      ...
      className="portfolio-floating-nav fixed top-1/2 z-40 hidden -translate-y-1/2 lg:block"
    >
          ...
          return (
            <Tooltip key={item.id}>
              <TooltipTrigger
                ...
                className={cn(
                  "flex size-11 cursor-pointer items-center justify-center rounded-full border border-transparent text-slate-300 transition-[background-color,border-color,color,box-shadow,transform] duration-75 ease-out hover:scale-105 hover:border-cyan-200/20 hover:bg-white/8 hover:text-cyan-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-200 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950",
                  isActive &&
        ...
        {navItems.map((item) => {
          ...
          return (
            <a
              ...
              aria-current={isActive ? "page" : undefined}
              className={cn(
                "flex size-10 cursor-pointer items-center justify-center rounded-full text-slate-500 transition hover:bg-slate-950/6 hover:text-slate-950 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 dark:text-slate-400 dark:hover:bg-white/8 dark:hover:text-cyan-100",
                isActive &&
